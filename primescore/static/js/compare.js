(function (PrimeScoreApp) {
  ...
  function buildTeamInputs() {
    ...
    const count = parseInt(
      PrimeScoreApp.getById("teamCompareCount")?.value || "2",
      10,
    );
    if (!container) return;
  ...
  function wireTeamAutocomplete(index) {
    ...
    const search = PrimeScoreApp.debounce(async (query) => {
      ...
      try {
        const leagueFilter =
          PrimeScoreApp.getById("teamLeagueFilter")?.value || "";
  ...
  function updatePlayerCompareInputs() {
    ...
    const count = parseInt(
      PrimeScoreApp.getById("playerCompareCount")?.value || "2",
      10,
    );
    if (!container) return;
  ...
  function wirePlayerSlot(index) {
    ...
    // Clear button
    PrimeScoreApp.getById(`playerSlotClear${index}`)?.addEventListener(
      "click",
      () => {
        delete slot.dataset.playerId;
        PrimeScoreApp.getById(`playerSlotSelected${index}`).style.display =
          "none";
        PrimeScoreApp.getById(`playerSlotPicker${index}`).style.display =
          "block";
        const teamInput = PrimeScoreApp.getById(`playerTeamSearch${index}`);
        if (teamInput) teamInput.value = "";
        PrimeScoreApp.getById(`playerSquadPanel${index}`).style.display =
          "none";
      },
    );
    ...
    const search = PrimeScoreApp.debounce(async (query) => {
      ...
      try {
        ...
        showDropdown(
          ...
          teams,
          async (team) => {
            await loadSquadForSlot(index, team);
          },
  ...
  async function loadSquadForSlot(index, team) {
    ...
    try {
      ...
      renderSquad(squadList, players, (player) => {
        ...
        PrimeScoreApp.getById(`playerSlotName${index}`).textContent =
          player.name;
        PrimeScoreApp.getById(`playerSlotSelected${index}`).style.display =
          "block";
        PrimeScoreApp.getById(`playerSlotPicker${index}`).style.display =
  ...
  async function compareTeamsMultiple() {
    const resultEl = PrimeScoreApp.getById("teamComparisonResult");
    ...
    if (resultEl) resultEl.innerHTML = "<p>Loading team comparison…</p>";
    ...
    try {
      ...
      const standings = leagueFilter
        ? await PrimeScoreApp.fetchStandings(leagueFilter)
        : [];
      ...
      if (resultEl) {
        resultEl.innerHTML = renderComparisonTable(
          rows,
          [
      ...
      if (resultEl)
        resultEl.innerHTML = `<p class="error">${PrimeScoreApp.escapeHtml(error.message || "Team comparison failed.")}</p>`;
    }
  }

  async function comparePlayersMultiple() {
    const resultEl = PrimeScoreApp.getById("playerComparisonResult");
    const count = parseInt(
      PrimeScoreApp.getById("playerCompareCount")?.value || "2",
      10,
    );
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!id) {
        if (resultEl)
          resultEl.innerHTML = `<p class="error">Select a player for slot ${label}.</p>`;
        return;
      }
      if (playerIds.includes(id)) {
        if (resultEl)
          resultEl.innerHTML = `<p class="error">Choose different players for comparison.</p>`;
        return;
      }
      playerIds.push(id);
    }

    if (resultEl) resultEl.innerHTML = "<p>Loading player comparison…</p>";

    try {
      const responses = await Promise.all(
        playerIds.map((id) =>
          PrimeScoreApp.apiFetch(`/api/players/${id}/statistics`),
        ),
      );
      ...
      if (resultEl)
        resultEl.innerHTML = `<p class="error">${PrimeScoreApp.escapeHtml(error.message || "Player comparison failed.")}</p>`;
    }
  }
