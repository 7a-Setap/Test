(function (PrimeScoreApp) {
      ...
      squadList.querySelectorAll(".squad-player-btn").forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          btn.classList.toggle(
            "squad-player-added",
            editState.players.some((p) => String(p.id) === String(btn.dataset.id)),
          );
        });
      });
    } catch {
  ...
  const searchPlayersTeam = PrimeScoreApp.debounce(async function (query) {
    ...
    try {
      ...
      const items = (data.teams || []).map((t) => ({ id: t.id, name: t.name, crest: t.crest }));
      showSuggestions("favPlayersTeamSuggestions", "favPlayersTeamSearch", items, (team) => {
  ...
  const searchLeagues = PrimeScoreApp.debounce(async function (query) {
    ...
    try {
      ...
      showSuggestions("favLeaguesSuggestions", "favLeaguesSearch", items, (item) =>
  ...
  function wireSearchInputs() {
    const configs = [
      ...
      { inputId: "favPlayersTeamSearch", suggestionsId: "favPlayersTeamSuggestions", handler: searchPlayersTeam },
  ...
  async function saveFavourites(event) {
    ...
    try {
      ...
      if (PrimeScoreApp.state) {
        const lastLeague = editState.leagues.at(-1);
        PrimeScoreApp.state.homeLeagueCode = lastLeague ? String(lastLeague.id) : "";
        PrimeScoreApp.state.homeLeagueName = lastLeague ? lastLeague.name : "";
      }
